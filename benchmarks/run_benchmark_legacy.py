...
class Vectorizer:
    def __init__(self, model_path: str, device=None, is_hyperbolic=False, target_dim=None):
        ...
        if is_hyperbolic:
            self.model = HyperV5Model(base_name)
...
def calculate_brute_force_gt(query_vecs: np.ndarray, doc_vecs: np.ndarray, doc_ids: List[str], k: int, metric: str) -> List[List[str]]:
    ...
    if metric_l == "cosine":
        # Safe normalization to avoid warnings
        doc_vecs = doc_vecs.astype(np.float32)
        doc_vecs = np.nan_to_num(doc_vecs, nan=0.0, posinf=0.0, neginf=0.0)
        
        doc_norms = np.linalg.norm(doc_vecs, axis=1, keepdims=True)
        # Avoid division by zero warnings
        doc_norms[doc_norms < 1e-12] = 1.0 
        doc_normed = doc_vecs / doc_norms
        
        # Ensure queries are clean too
        query_vecs = query_vecs.astype(np.float32)
        query_vecs = np.nan_to_num(query_vecs, nan=0.0, posinf=0.0, neginf=0.0)
        
        for q in tqdm(query_vecs, desc="Brute-force GT (cosine)"):
            q_norm = np.linalg.norm(q)
            qn = q / q_norm if q_norm > 1e-12 else q
...
def run_benchmark():
    ...
    if cfg.target_case:
        ...
        if train_path.exists():
            ...
            # Keep dataset-native IDs when available, otherwise fallback to positional IDs.
            if "id" in df_train.columns:
                train_ids = [str(x) for x in df_train["id"].values[: len(doc_vecs_euc)]]
            else:
                train_ids = [str(i) for i in range(len(doc_vecs_euc))]
        ...
        if test_path.exists():
            ...
            q_ids_from_data = [str(x) for x in df_test["id"].values] if "id" in df_test.columns else None
        ...
        if q_vecs_euc is not None:
            ...
            test_queries = [f"Query {i}" for i in range(len(q_vecs_euc))]
            if test_path.exists() and "q_ids_from_data" in locals() and q_ids_from_data is not None:
        ...
        if neighbors_path.exists():
             ...
             # Support known GT column variants from vectordb-bench datasets
             col = None
    ...
    if CHROMA_AVAILABLE and (not target_db or "chroma" in target_db):
        ...
        try:
            ...
            if chroma_local_dir:
                disk = get_local_disk(chroma_local_dir)
            else:
