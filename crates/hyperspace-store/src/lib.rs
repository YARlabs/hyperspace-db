...
impl VectorStore {
    pub fn new(base_path: &Path, element_size: usize) -> Self {
        ...
        loop {
            ...
            if !path.exists() {
                if i == 0 {
                   let seg = Self::create_segment(&path, element_size).expect("Failed to create init segment");
    ...
    pub fn append(&self, vector_bytes: &[u8]) -> Result<u32, String> {
        if vector_bytes.len() != self.element_size {
             return Err(format!("Vector size mismatch: {} vs {}", vector_bytes.len(), self.element_size));
        }
