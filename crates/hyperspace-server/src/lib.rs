...
#[path = "audit_log.rs"]
pub mod audit_log;
#[path = "chunk_backend.rs"]
pub mod chunk_backend;
#[path = "chunk_searcher.rs"]
...
pub use crate::server_args::Args;
...
#[cfg(feature = "depin")]
fn verify_grpc_ticket<T>(
    billing: &hyperspace_billing::BillingContext,
    req: &tonic::Request<T>,
) -> Result<(), tonic::Status> {
    // 1. Get the ticket from metadata
    let ticket_header = match req.metadata().get("x-hs-ticket") {
        Some(val) => match val.to_str() {
            Ok(s) => s,
            Err(_) => {
                return Err(tonic::Status::unauthenticated(
                    "x-hs-ticket metadata is not a valid string",
                ))
            }
        },
        None => {
            return Err(tonic::Status::unauthenticated(
                "Missing x-hs-ticket metadata",
            ))
        }
    };

    // 2. Decode the ticket from base64
    use base64::{engine::general_purpose::STANDARD, Engine};
    let ticket_bytes = match STANDARD.decode(ticket_header) {
        Ok(bytes) => bytes,
        Err(e) => {
            return Err(tonic::Status::unauthenticated(format!(
                "Failed to base64-decode x-hs-ticket: {e}"
            )))
        }
    };

    // 3. Deserialize JSON SignedTicket
    let ticket: hyperspace_billing::SignedTicket = match serde_json::from_slice(&ticket_bytes) {
        Ok(t) => t,
        Err(e) => {
            return Err(tonic::Status::unauthenticated(format!(
                "Failed to deserialize SignedTicket JSON: {e}"
            )))
        }
    };

    // 4. Verify the ticket
    let mut verifier = billing
        .ticket_verifier
        .lock()
        .map_err(|_| tonic::Status::internal("Internal error locking ticket verifier"))?;
    if let Err(e) = verifier.verify(&ticket) {
        return Err(tonic::Status::unauthenticated(format!(
            "Ticket verification failed: {e}"
        )));
    }

    // 5. Store verified ticket in PendingTicketStore
    if let Err(e) = billing.ticket_store.push(&ticket) {
        return Err(tonic::Status::internal(format!(
            "Failed to persist SignedTicket to redb queue: {e}"
        )));
    }
            ...
            async {
                ...
                if let Some(billing) = &self.billing {
                    ...
                    if !ctx.user_id.is_empty() && ctx.user_id != "anonymous" {
                        billing.metering.record_insert(&ctx.user_id, 1);

                        // Update storage bytes so SyncWorker can bill storage rental each tick.
                        let approx_storage_bytes = col.count() as u64 * col.dimension() as u64 * 4; // f32 = 4 bytes per component
                        billing
                            .metering
                            .update_storage_bytes(&ctx.user_id, approx_storage_bytes);
                    }
                }
                ...
                Ok(res) => {
                    ...
                    if let Some(billing) = &self.billing {
                        ...
                        if !api_key_header.is_empty() {
                            ...
                            billing
                                .metering
                                .record_search(&api_key_header, payload_bytes);
                        }
                    }
...
pub async fn start_server(
    args: Args,
    #[cfg(feature = "depin")] billing_override: Option<hyperspace_billing::BillingContext>,
    ...
    let billing_ctx = {
        let resolved = match billing_override {
            ...
            None if std::env::var("HS_DEPIN_MODE").as_deref() == Ok("1") => {
                ...
                match hyperspace_billing::BillingContext::start(
                    &db_path,
                    coordinator.clone(),
                    max_rps,
                    interval,
...
pub async fn main_entry() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
