...
#[cfg(feature = "depin")]
fn verify_depin_ticket<T>(
    billing_opt: &Option<Arc<hyperspace_billing::BillingContext>>,
    request: &Request<T>,
    user_id: &str,
) -> Result<(), Status> {
    ...
    // 1. Billing enforcement (is_throttled check)
    if billing.metering.is_throttled(user_id) {
        return Err(Status::resource_exhausted(
            "Insufficient balance — top up your account to continue",
        ));
    }

    // 2. Extract x-hs-ticket header
    let ticket_header = request.metadata().get("x-hs-ticket");
    let ticket_str = ticket_header
        .and_then(|v| v.to_str().ok())
        .ok_or_else(|| Status::unauthenticated("Missing x-hs-ticket metadata header"))?;
    
    use base64::{engine::general_purpose::STANDARD, Engine};
    let ticket_bytes = STANDARD.decode(ticket_str)
        .map_err(|e| Status::invalid_argument(format!("Invalid base64 in x-hs-ticket: {e}")))?;
    let ticket: hyperspace_billing::SignedTicket = serde_json::from_slice(&ticket_bytes)
        .map_err(|e| Status::invalid_argument(format!("Invalid json in x-hs-ticket: {e}")))?;

    // 3. Verify signature & replay
    let mut verifier = billing.ticket_verifier.lock().unwrap();
    verifier.verify(&ticket)
        .map_err(|e| Status::unauthenticated(format!("Ticket verification failed: {e}")))?;

    // 4. Save ticket to pending store
    billing.ticket_store.push(&ticket)
        .map_err(|e| Status::internal(format!("Failed to store ticket: {e}")))?;
...
pub struct HyperspaceService {
    ...
    #[cfg(feature = "depin")]
    pub billing: Option<Arc<hyperspace_billing::BillingContext>>,
}
...
impl Database for HyperspaceService {
    ...
    async fn insert(
        ...
        // ─── Phase C: Ticket verification ─────────────────────────────────────────
        #[cfg(feature = "depin")]
        verify_depin_ticket(&self.billing, &request, &ctx.user_id)?;
    ...
    async fn batch_insert(
        ...
        let result: Result<Response<InsertResponse>, Status> =
            async {
                ...
                let total_inserted = vectors.len() as u64;
                ...
                if let Some(billing) = &self.billing {
                    let m = &billing.metering;
                    if !ctx.user_id.is_empty() && ctx.user_id != "anonymous" {
                        m.record_insert(&ctx.user_id, total_inserted);
        ...
        for (idx, search_req) in req.searches.into_iter().enumerate() {
            ...
            tasks.spawn(async move {
                ...
                let results = res
                    ...
                    .collect();
                Ok::<(std::primitive::usize, SearchResponse), Status>((idx, SearchResponse { results }))
            });
        }
...
pub async fn start_server(
    args: Args,
    #[cfg(feature = "depin")] billing_override: Option<Arc<hyperspace_billing::BillingContext>>,
    ...
    #[cfg(feature = "depin")]
    let billing_override = {
        let resolved = match billing_override {
            Some(b) => Some(b),
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    start_server(
        args,
        #[cfg(feature = "depin")]
        None, // billing is None in standard server mode; hyperspace-miner injects it
    ).await
