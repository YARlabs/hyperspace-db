...
impl HyperspaceDB {
    ...
    pub async fn save(&self) -> Result<(), JsValue> {
        ...
        let transaction = rexie.transaction(&[STORE_NAME], TransactionMode::ReadWrite)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;
        let store_os = transaction.store(STORE_NAME)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;
        ...
        let index_bytes = self.index.save_to_bytes()
            .map_err(|e| JsValue::from_str(&e))?;
        let index_js = serde_wasm_bindgen::to_value(&index_bytes)?;
        store_os.put(&index_js, Some(&JsValue::from_str("index"))).await
            .map_err(|e| JsValue::from_str(&e.to_string()))?;
        
        // 3. Export ID Maps
        let id_map = self.id_map.read();
        let map_js = serde_wasm_bindgen::to_value(&*id_map)?;
        store_os.put(&map_js, Some(&JsValue::from_str("id_map"))).await
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        transaction.done().await.map_err(|e| JsValue::from_str(&e.to_string()))?;
    ...
    pub async fn load(&mut self) -> Result<bool, JsValue> {
        ...
        let transaction = rexie.transaction(&[STORE_NAME], TransactionMode::ReadOnly)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;
        let store_os = transaction.store(STORE_NAME)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        // Retrieve Vectors
        let vectors_js = store_os.get(&JsValue::from_str("vectors")).await
            .map_err(|e| JsValue::from_str(&e.to_string()))?;
        ...
        // Retrieve Index
        let index_js = store_os.get(&JsValue::from_str("index")).await
            .map_err(|e| JsValue::from_str(&e.to_string()))?;
        let index_bytes: Vec<u8> = serde_wasm_bindgen::from_value(index_js)?;

        // Retrieve ID Map
        let map_js = store_os.get(&JsValue::from_str("id_map")).await
            .map_err(|e| JsValue::from_str(&e.to_string()))?;
        ...
        let storage = Arc::new(VectorStore::from_bytes(std::path::Path::new("mem"), element_size, &vectors_bytes));
        
        let config = Arc::new(GlobalConfig::default());
        
        // 2. Restore Index
        let index = MyIndex::load_from_bytes(
            &index_bytes,
            storage,
            QuantizationMode::None,
            config
