...
impl Interceptor for AuthInterceptor {
    fn call(&mut self, mut request: Request<()>) -> Result<Request<()>, Status> {
        request
            .metadata_mut()
            .insert("x-api-key", "I_LOVE_HYPERSPACEDB".parse().unwrap());
        Ok(request)
    }
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let found_1 = results
        .iter()
        .find(|r| r.id == 1)
        .expect("Should find Internal ID 1");
    // Tolerance due to quantization (1/127 ~ 0.008)
    assert!(
        (found_1.distance - 0.25).abs() < 0.02,
        "Internal ID 1 distance should be 0.25, got {}",
        found_1.distance
    );
    ...
    let found_2 = results
        .iter()
        .find(|r| r.id == 2)
        .expect("Should find Internal ID 2");
    assert!(
        (found_2.distance - 0.64).abs() < 0.02,
        "Internal ID 2 distance should be 0.64, got {}",
        found_2.distance
    );
