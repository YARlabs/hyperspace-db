...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let collection_name = format!("full_test_{}", std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .unwrap()
        .as_secs());
    
    println!("📦 Creating test collection: {}...", collection_name);
    leader.create_collection(
    ...
    println!("\n📊 After 10 inserts:");
    println!("   Leader:   hash={}, count={}", leader_digest.state_hash, leader_digest.count);
    println!("   Follower: hash={}, count={}", follower_digest.state_hash, follower_digest.count);
    
    assert_eq!(leader_digest.state_hash, follower_digest.state_hash, "Hashes should match after 10 inserts");
    ...
    println!("   Leader:   hash={}, count={}", leader_digest2.state_hash, leader_digest2.count);
    println!("   Follower: hash={}, count={}", follower_digest2.state_hash, follower_digest2.count);
    
    assert_eq!(leader_digest2.state_hash, follower_digest2.state_hash, "Hashes should match after 15 inserts");
    assert_eq!(leader_digest2.count, 15, "Leader should have 15 vectors");
    assert_eq!(follower_digest2.count, 15, "Follower should have 15 vectors");
    println!("   ✅ Test 2 PASSED");
    
    // Test 3: Verify bucket-level sync
    println!("\n📝 Test 3: Verifying bucket-level sync...");
    let mut matching_buckets = 0;
    for (i, (l_bucket, f_bucket)) in leader_digest2.buckets.iter()
        ...
        .enumerate() 
    {
        if l_bucket == f_bucket {
            ...
            println!("   ⚠️  Bucket {} mismatch: Leader={}, Follower={}", i, l_bucket, f_bucket);
        }
    }
